...
public class ManualSelectByGroupsController extends FXController {
    ...
    public ManualSelectByGroupsController(ScreensConfig config) {
		super(config);
	}
    ...
    private void setSchedule(ScheduleInterface scheduleInterface) {
        ...
        indexColumn.setCellValueFactory(column -> new ReadOnlyObjectWrapper<>(ScheduleTable.getItems().indexOf(column.getValue()) + 1).asString());
